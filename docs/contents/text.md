...
数字和英文单位或符号单位的组合与全角中文字符间，应有半角空格。
```
...
外文缩写可以使用半角圆点(`.`)表示缩写，此时不需要在标点后加半角空格。
...
```
A Brief Introduction to Nervos Network
```

 冠词、介词、连词首字母都不需要大写（除非是标题中的第一个单词）

```
Mastering Nervos with Mr. Jan
How to Survive a Faster Chaging Future
```

 字母多于三个（不含三个）的介词、连词首字母要大写，比如 through

```
Mastering Nervos Through Mr. Jan
```
